...
真实联调时改为 `AGENT_MODE=deepseek` 并配置 `DEEPSEEK_API_KEY`，重启后端。模型固定为 DeepSeek 官方 `deepseek-flash`，显式 `thinking.type=disabled`，不自动切换供应商。日常开发默认 mock；新版的真实模型和 OAuth 验收须按发布手册单独执行。
...
知乎 OAuth 的授权、Token、用户信息端点和身份字段必须按合作方官方文档填写；初始配置没有猜测任何知乎端点，当时尚未进行真实登录联调；后续记录见 [知乎 OAuth 联调与部署](docs/zhihu-oauth-deployment.md)。OAuth 凭据不存入浏览器，本站使用可撤销的 HttpOnly Cookie 会话。
...
贡献流程与开发约定见 [CONTRIBUTING.md](CONTRIBUTING.md)（英文）。
