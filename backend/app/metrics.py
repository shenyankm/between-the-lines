...
Counters reset on process restart; durable usage lives in turns. Mutations run
on the single event loop without awaits. Replays have their own counter and do
not contribute model usage, cost or execution duration.
"""
...
class Metrics:
    ...
    def observe_turn(
        self, status: str, duration_seconds: float, model_calls: int, cost_usd: float
    ) -> None:
        ...
        if status == "replayed":
            return
        self._model_calls_total += max(0, model_calls)
        self._cost_usd_total += max(0.0, cost_usd)
    ...
    def _lines(self, active_turns: int) -> Iterator[str]:
        ...
        yield "# HELP btl_turns_active Turns holding a slot in the admission budget."
        yield "# TYPE btl_turns_active gauge"
        yield f"btl_turns_active {active_turns}"
        yield "# HELP btl_model_calls_total DeepSeek-compatible model calls made."
        yield "# TYPE btl_model_calls_total counter"
        yield f"btl_model_calls_total {self._model_calls_total}"
        yield "# HELP btl_cost_usd_total Estimated spend, zero while in mock mode."
        yield "# TYPE btl_cost_usd_total counter"
        yield f"btl_cost_usd_total {self._cost_usd_total:.8f}"
