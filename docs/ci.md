...
Web uses digest-pinned Nginx 1.30.4 / Alpine with security updates, running as UID/GID 101. PID and temporary data use /tmp; internal ports are 8080/8443, with unchanged external Compose ports. See [development and deployment](development.md) for TLS key permissions. `scripts/test-web-runtime.sh` uses isolated temporary certificates and volumes to verify non-root operation, HTTP redirects, HTTPS static pages, and API proxying.
