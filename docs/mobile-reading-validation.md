...
The mobile stage now places dialogue and input before secondary tools in both DOM and visual order. All existing tools, save navigation and logout remain available. Focused input reduces portrait space without clipping dialogue. Stored reading size and reveal speed still apply; reduced motion always reveals complete text. Receipt details collapse inside a tool panel so forms remain reachable.

Automated coverage uses Mock data, Chromium desktop and a 390 px touch viewport, plus 320 px (equivalent narrow reflow), landscape, long Chinese input and 44 px primary controls. These are browser simulations, not physical keyboard or screen-reader results. Before/after baseline images are linked in issue #34; updated desktop/mobile screenshots are attached to its PR.
...
- [ ] VoiceOver and TalkBack: reading order, recipient label, status announcements, panel focus/close, return home, and the confirmations before leaving or logging out.
