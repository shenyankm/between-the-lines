...
| ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------- |
| Free story actions, legacy request_id replay, recently played                      | `backend/app/services.py`, `routes/game.py`, `budget.py`                                                                      |
...
| 0007      | Independent AI cost ledger, backfilled from new tasks and legacy AI turns; deleting saves does not remove costs from monthly budgets                                |

Migrations add fields/tables without rewriting v1 state JSON or existing turn payload/result. Legacy five-field inputs still compare their original fields for idempotency; omitted new fields do not participate. v1 cannot upgrade mid-story or gain fabricated historical snapshots. Branches copy facts, dialogue, and private narrative within the snapshot boundary using new event IDs, history groups, and checkpoint namespaces. They do not copy tool checkpoints, model costs, or later content. Parent saves are provenance only; deleting one does not delete independent branches.
...
2. Prepare a compatible backend supporting schema 0004–0007. Migrate before switching applications, retaining one instance and one API worker. Never run incompatible old writers simultaneously.
...
Disabling automatic intents returns to buttons. Disabling new v2 stories blocks creation only; existing v2 saves can continue. Disabling perspective generation shows editorial advice. Roll back only to an application build compatible with the current schema; do not destructively downgrade the database. Migration-test downgrades are only for dedicated empty databases. Retain additive migrations and fix forward when necessary.

## Identity and progress

Guest cookies are HttpOnly and SameSite=Lax, plus Secure in production; identities last seven days. Limits are five new identities per IP per hour, eight AI tasks by default, and one trial save including archived/recycled saves. Zero AI quota still permits Act 1 actions. Act 2 requires member login. Denying authorization does not erase guest progress.

At authorization initiation, the source guest is stored in server-side OAuthBinding associated with Authlib state. The callback does not infer it from the browser's current cookie. Linking either new or existing Zhihu accounts transfers ownership of saves, turns, AI tasks, and costs without overwriting target saves, resetting budgets, or changing namespaces. Running turns/tasks defer migration, with a pending indication on Home. Restart resumes pending migrations; completion revokes guest sessions.

## AI budgets and recovery

All AI use reserves a ledger entry within a database transaction; a transaction lock serializes global-budget admission. Model calls run outside transactions. Reservations use request UTF-8 byte limits, output limits, call counts, and retries; actual HTTP requests exceeding byte/count limits are rejected. Timeouts and failures retain unconfirmed costs instead of releasing them as zero. Known token costs use configured estimates, and monthly totals include recorded legacy costs. Provider billing reconciliation is still required.

Member defaults are 100 AI tasks per UTC day; guests receive eight total. Deterministic actions create no cost ledger but remain endpoint-rate-limited. Card cache hits and editorial advice consume no AI task quota. Perspective cards and reflections allow one generation plus one structural repair. Restart persists interrupted generation as failed/unknown usage with fact-based fallback. Repeating request_id returns the same result without hidden rebilling.

To reconcile unknown costs, prepare `[{"id":"cost-ledger-UUID","cost_usd":0.012}]`, dry-run first, then explicitly use `--apply`. Values must come from provider records; do not estimate costs away to zero.
...
Archiving frees slots from the member limit of twenty active saves. Recycled saves are retained for thirty days; expired unbound guests receive seven additional days. cleanup reports counts by default. Explicit apply removes expired saves, derived business data, and each NPC's checkpoint records. Running or pending-binding saves are excluded. Cost ledgers and minimal identity tombstones remain to prevent cleanup from resetting spending. Proactive checkpoint compaction for normally completed stories is not enabled.

Raw product events are retained thirty days, aggregates 180 days. Diagnostics accept only allowlisted fields and static-asset stack locations; raw chats, names, and OAuth code/state are not collected by default. Feedback is explicitly submitted player text. metrics reports funnels, failures, mean/p95 waits, and estimated costs. Estimated completion cost divides period spending by completed saves in that period; it is an operational indicator, not exact per-player attribution.
...
# Real models require explicit opt-in, capped at an estimated 2 USD; incomplete runs report remaining work and fail.
